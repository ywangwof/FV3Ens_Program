...
import cape_routines as cape_calc
...
#      cape_0to3_100mb = np.zeros((ne,ny,nx))                      #lowest 100mb mixed layer 0-3 km AGL CAPE (J/kg)
      cape_sfc = np.zeros((ne,ny,nx))
      cin_sfc = np.zeros((ne,ny,nx))
      lcl_sfc = np.zeros((ne,ny,nx))
      lfc_sfc = np.zeros((ne,ny,nx))
      cape_mu = np.zeros((ne,ny,nx))
      cin_mu = np.zeros((ne,ny,nx))
      lcl_mu = np.zeros((ne,ny,nx))
      lfc_mu = np.zeros((ne,ny,nx))
...
############################### Read WRFOUT variables: ##################################
   ...
   stemps = temps[0,:,:]+6.5*fin.variables['HGT'][0,:,:]/1000.
   mslp[f,:,:] = fin.variables['PSFC'][0,:,:]*np.exp(9.81/(287.0*stemps)*fin.variables['HGT'][0,:,:])*0.01 + (6.7 * fin.variables['HGT'][0,:,:] / 1000.)
...
#   pbl_hgt[f,:,:] = fin.variables["PBLH"][0,:,:]       #not in WRFWOF files yet
   sw_down[f,:,:] = fin.variables["SWDOWN"][0,:,:]
   iwp[f,:,:] = fin.variables["IWP"][0,:,:]
   lwp[f,:,:] = fin.variables["LWP"][0,:,:]
...
#   cin_100mb[f,:,:] = calc_cin(t_v, t_100mb_parcel, p, lfc_p_100mb, dz)
   ...
   ter_follow = 1 #1=terrain following coordinates
   cape_100mb[f,:,:],cin_100mb[f,:,:],lcl_100mb[f,:,:],lfc_100mb[f,:,:] = cape_calc.mlcape2(p,temp,qv,z,hgt,p_sfc_temp,ter_follow)
   cape_mu[f,:,:],cin_mu[f,:,:],lcl_mu[f,:,:],lfc_mu[f,:,:] = cape_calc.mucape(p,temp,qv,z,hgt,p_sfc_temp,ter_follow)
   cape_sfc[f,:,:],cin_sfc[f,:,:],lcl_sfc[f,:,:],lfc_sfc[f,:,:] = cape_calc.sfcape(p,temp,qv,z,hgt,p_sfc_temp,ter_follow)

   cin_100mb[f,:,:] = -cin_100mb[f,:,:]
...
lcl_mu_var = fout.createVariable('lcl_mu', 'f4', ('NE','NY','NX',))
lcl_mu_var.long_name = "most unstable lifted condensation level height"
lcl_mu_var.units = "m"

lfc_mu_var = fout.createVariable('lfc_mu', 'f4', ('NE','NY','NX',))
lfc_mu_var.long_name = "most unstable level of free convection"
lfc_mu_var.units = "m"

lcl_sf_var = fout.createVariable('lcl_sfc', 'f4', ('NE','NY','NX',))
lcl_sf_var.long_name = "surface based lifted condensation level height"
lcl_sf_var.units = "m"

lfc_sf_var = fout.createVariable('lfc_sfc', 'f4', ('NE','NY','NX',))
lfc_sf_var.long_name = "surface based level of free convection"
lfc_sf_var.units = "m"
...
cape_mu_var = fout.createVariable('cape_mu', 'f4', ('NE','NY','NX',))
cape_mu_var.long_name = "Most unstable CAPE (lowest3 km)"
cape_mu_var.units = "J/Kg"

cin_mu_var = fout.createVariable('cin_mu', 'f4', ('NE','NY','NX',))
cin_mu_var.long_name = "Most unstable parcel convective inhibition (lowest 3 km)"
cin_mu_var.units = "J/Kg"

cape_sfc_var = fout.createVariable('cape_sfc', 'f4', ('NE','NY','NX',))
cape_sfc_var.long_name = "100 hPa mixed layer CAPE"
cape_sfc_var.units = "J/Kg"

cin_sfc_var = fout.createVariable('cin_sfc', 'f4', ('NE','NY','NX',))
cin_sfc_var.long_name = "100 hPa mixed layer convective inhibition"
cin_sfc_var.units = "J/Kg"
...
#mu
fout.variables['lcl_mu'][:] = lcl_mu
fout.variables['lfc_mu'][:] = lfc_100mb
fout.variables['cape_mu'][:] = cape_mu
fout.variables['cin_mu'][:] = cin_mu
#sfc
fout.variables['lcl_sfc'][:] = lcl_sfc
fout.variables['lfc_sfc'][:] = lfc_sfc
fout.variables['cape_sfc'][:] = cape_sfc
fout.variables['cin_sfc'][:] = cin_sfc
