...
if (process == 1):
      if (current_t >= 3):
         cmd = "python news_e_post_ensemble_rainfall.py -d %s -o %s -t %d " % (fcst_dir, outdir, (current_t-3))
...
cmd = "python news_e_post_ensemble_rainfall.py -d %s -o %s -t %d " % (fcst_dir, outdir, (current_t-2))
pool.apply_async(run_script, (cmd,))

time.sleep(10)

cmd = "python news_e_post_ensemble_rainfall.py -d %s -o %s -t %d " % (fcst_dir, outdir, (current_t-1))
pool.apply_async(run_script, (cmd,))

time.sleep(30)

cmd = "python news_e_post_ensemble_rainfall.py -d %s -o %s -t %d " % (fcst_dir, outdir, (current_t))
