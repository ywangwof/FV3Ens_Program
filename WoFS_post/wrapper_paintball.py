...
while ((ens_t < fcst_nt) and (pcp_ens_t < fcst_nt)):
   summary_files_temp = os.listdir(summary_dir)

   for f, file in enumerate(summary_files_temp):
      str_ens_t = str(ens_t)
      str_pcp_ens_t = str(pcp_ens_t)
      if (len(str_ens_t) == 1):
         str_ens_t = '0' + str_ens_t

      if (len(str_pcp_ens_t) == 1):
         str_pcp_ens_t = '0' + str_pcp_ens_t
...
#            pool.apply_async(run_script, (cmd,))
      ...
      if ((file[-28:-25] == 'PCP') and (file[-24:-22] == str_pcp_ens_t)):
         time.sleep(3)

         cmd = "python news_e_paintball_qpf.py -d %s -o %s -t %d -n %d " % (summary_dir, outdir, pcp_ens_t, fcst_nt)
         pool.apply_async(run_script, (cmd,))

         pcp_ens_t = pcp_ens_t + 1
...
#cmd = "python news_e_paintball_svr.py -d %s -o %s -t %d -n %d " % (summary_dir, outdir, ens_t, fcst_nt)
#pool.apply_async(run_script, (cmd,))
