...
if __name__ == "__main__":
    ...
    # Read FV3 grid
    #
    fh = Dataset(files[0]['MPH'][memlist[0]]['phyf'],'r')
    ...
    known = np.vstack((np.ravel(lon2d), np.ravel(lat2d))).T
    ...
    t = 0
    for i in range(0,fmin+imin,imin):
        currtime = inittime+timedelta(minutes=i)
        #
        ...
        validobs = process_obs(sfcobs,lat2d,lon2d)

        for n,case in enumerate(casenames):
            ds_dict = load_Dataset(files[t][case])

            for v, var in enumerate(varens[fileindx]):
                ask = np.vstack((validobs[var]['lon'],validobs[var]['lat'])).T
                ...
                for m,mem in enumerate(memlist):
                    print(f"Computing {case}:{mem} for {var} at {i} min" )
                    fh = ds_dict[mem][fileindx]
                    if var == 'td2m':
                    ...
                    invdisttree   = INVTree( known, np.ravel(vardata), leafsize=leafsize, stat=1 )
                    varobs = invdisttree( ask, nnear=Nnear, eps=eps, p=p )  # dimensioned by number of observations
                    #plot_regular(vardata,f"{var}_grid",i,fh)
                    #plot_irregular(varobs,validobs[var]['lat'],validobs[var]['lon'],f"{var}_obs",i,fh)
                    vardiff += np.sum( (varobs-validobs[var]['OBS'])**2 )
                    varbias += np.sum( (varobs-validobs[var]['OBS'])    )
                    varcnt  += len(varobs)
                    var2d[m,:] = vardata.ravel()

                varstd = var2d.std(axis=0)         # std over each grid point over ensemble members
                var_stds[v,n,t] = varstd.mean()
                var_rmse[v,n,t] = np.sqrt( vardiff / varcnt )
                var_bias[v,n,t] = varbias / varcnt
        t += 1

    outfile = f"/oldscratch/ywang/EPIC/Program/plots/stat2m.{initdate}.npy"
