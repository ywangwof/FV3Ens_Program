...
if __name__ == "__main__":
    ...
    print(f"\nInvdisttree:  N {N}  Ndim {Ndim}  Nask {Nask}  Nnear {Nnear}  leafsize {leafsize}  eps {eps}  p {p}")
    ...
    print(f"average distances to nearest points: {np.mean( invdisttree.distances, axis=0)}" )

    print(f"average weights: {invdisttree.wsum / invdisttree.wn}")
    # see Wikipedia Zipf's law
    err = np.abs( terrain(ask) - interpol )
    print(f"average |terrain() - interpolated|: {np.mean(err)}")
